...
def computeAll(H_true):
    ...
    for t in range(5):
        ...
        for t_b in range(5):
            ...
            dist = []
            for i in range(m):
                distance = np.linalg.norm(np.dot(A_hat_true[0], exemplar) - np.dot(A_true[0], X[i, :]), ord=1)
                dist.append(round(distance, 3))

            for i in range(m):
                if dist[i] + epsilon <= round(median(dist), 3):
                    y[i] = 0
    ...
    columns = ['True H', 'In-sample mean', 'In-sample std', 'Out-of-sample mean', 'Out-of-sample std',
               'CrossVal mean H', 'CrossVal std H']
...
if __name__ == '__main__':
    ...
    df.to_csv('results_depth_1_parallel.csv', sep=',', index=False)
