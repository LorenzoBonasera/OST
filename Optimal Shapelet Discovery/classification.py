    ...
    if num_nodes > 1:
        for i in range(t_rows):
            if np.linalg.norm(np.dot(A_hat[0], exemplar) - np.dot(A[0], X_test[i, :]), ord=1) < b[0]:
                ...
                elif np.linalg.norm(np.dot(A_hat[1], exemplar) - np.dot(A[1], X_test[i, :]), ord=1) >= b[1]:
                    Y_predict[i, 1] = labels[1]

            elif np.linalg.norm(np.dot(A_hat[0], exemplar) - np.dot(A[0], X_test[i, :]), ord=1) >= b[0]:
                if np.linalg.norm(np.dot(A_hat[2], exemplar) - np.dot(A[2], X_test[i, :]), ord=1) < b[2]:
